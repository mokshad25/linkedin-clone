...
import { educationSchema, EducationValues } from '@/lib/validations/profile'
...
import { PlusIcon, Trash2Icon, Loader2Icon, GraduationCapIcon } from 'lucide-react'

interface Props { userId: string; initialItems: Education[] }

export default function EducationSection({ userId, initialItems }: Props) {
    ...
    const { register, handleSubmit, reset, watch, formState: { errors } } =
        useForm<EducationValues>({ resolver: zodResolver(educationSchema) })
    ...
    const onSubmit = async (values: EducationValues) => {
        ...
        if (error) { toast.error('Failed to add education'); setIsSaving(false); return }
        setItems([data, ...items])
    ...
    return (
        <div className="bg-white rounded-lg border border-slate-200 p-5" id="education">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 mb-5 p-4 bg-slate-50 rounded-lg border border-slate-200">
                    <div className="grid sm:grid-cols-2 gap-3">
                        <div className="space-y-1 sm:col-span-2">
                            ...
                            <Input placeholder="IIT Bombay" className="h-9 text-sm border-slate-200" {...register('school')} />
                        ...
                        <div className="space-y-1">
                            ...
                            <Input placeholder="B.Tech" className="h-9 text-sm border-slate-200" {...register('degree')} />
                        </div>
                        <div className="space-y-1">
                            <Label className="text-xs font-medium text-slate-700">Field of Study</Label>
                            <Input placeholder="Computer Science" className="h-9 text-sm border-slate-200" {...register('field_of_study')} />
                        </div>
                        <div className="space-y-1">
                        ...
                        <div className="space-y-1">
                            <div className="flex items-center gap-2 mb-1">
                                <input type="checkbox" id="edu_current" className="accent-[#0a66c2]" {...register('is_current')} />
                                <Label htmlFor="edu_current" className="text-xs text-slate-600 cursor-pointer">Currently studying</Label>
                            </div>
                            {!isCurrent && (
                    ...
                    <div className="space-y-1">
                        ...
                        <Textarea placeholder="Activities, achievements..." rows={2} className="text-sm border-slate-200 resize-none" {...register('description')} />
                    </div>
                    ...
                    <div key={item.id} className="flex gap-3 group">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-400">
                                {item.start_date ? formatDate(item.start_date) : ''}{item.end_date || item.is_current ? ` – ${item.is_current ? 'Present' : formatDate(item.end_date!)}` : ''}
                            </p>
                        </div>
