...
export default function ApplicantsClient({ job, applications: initialApps }: Props) {
    ...
    <div className="max-w-4xl mx-auto px-2 sm:px-4 py-4 space-y-4">
          ...
          {APPLICATION_STATUSES.map(({ value, label, color }) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${
                  filter === value ? 'ring-2 ring-offset-1 ring-slate-400 ' + color : color + ' opacity-70 hover:opacity-100'
                }`}
              >
                {label} ({count})
              </button>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-lg border border-slate-200 py-16 flex flex-col items-center gap-3 text-center">
          ...
          <p className="font-semibold text-slate-600">No applicants in this category</p>
        </div>
      ) : (
            ...
            return (
              <div key={app.id} className="bg-white rounded-lg border border-slate-200 p-4">
                <div className="flex items-start gap-3">
                  <Link href={`/u/${app.applicant?.username}`} className="shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <Link href={`/u/${app.applicant?.username}`} className="hover:underline">
                        ...
                        {app.applicant?.location && (
                          <div className="flex items-center gap-1 mt-0.5">
                            ...
                            <span className="text-xs text-slate-400">{app.applicant.location}</span>
                          </div>
                        )}
                      </div>
